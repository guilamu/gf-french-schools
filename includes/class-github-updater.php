...
class GF_French_Schools_GitHub_Updater
{
    ...
    private const PLUGIN_NAME = 'Gravity Forms - French Schools';
    private const PLUGIN_DESCRIPTION = 'Ajoute un champ "Écoles françaises" à Gravity Forms permettant de rechercher et sélectionner un établissement scolaire français via l\'API du Ministère de l\'Éducation Nationale.';
    private const REQUIRES_WP = '5.8';
    private const TESTED_WP = '6.7';
    private const REQUIRES_PHP = '7.4';
    private const REQUIRES_GF = '2.5';
    private const TEXT_DOMAIN = 'gf-french-schools';
     ...
     *
     * Prefers custom release assets (e.g., gf-french-schools.zip) over
    ...
    public static function plugin_info_css(): void
    {
        ...
        // Add "Requires Gravity Forms" line in the sidebar.
    ...
    private static function parse_readme(): array
    {
        ...
        $utility_sections = array(
            'changelog', 'change log', 'requirements', 'installation', 'faq',
            'prérequis', 'project structure', 'structure du projet',
            'acknowledgements', 'remerciements', 'license', 'licence',
            'auteur', 'author', 'contribuer', 'api utilisée',
        );
        ...
        for ($i = 1, $count = count($parts); $i < $count; $i++) {
            ...
            if ('installation' === $title) {
                ...
                $faq .= $body . "\n\n";
            } elseif ('changelog' === $title || 'change log' === $title) {
                $changelog .= $body . "\n\n";
    ...
    private static function markdown_to_html(string $markdown): string
    {
        ...
        // WordPress install_plugin_information() sanitizes with wp_kses()
        // using $plugins_allowedtags which does NOT include table/tr/th/td.
        // Convert <table> HTML to <div>/<span> structures that survive wp_kses.
     ...
     * Table elements are stripped entirely.
     *
